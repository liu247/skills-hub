      ...
      <div className="modal-body ai-parse-body">
        {!eligible.length ? <div className="ai-parse-notice"><TriangleAlert size={16}/>{t('aiParse.noProvider')}</div> : <>
          ...
          {!plan ? <button type="button" className="btn btn-primary" disabled={busy || !sourceUrl.trim() || !eligible.length} onClick={() => void parse()}><FileSearch size={15}/>{t('aiParse.parse')}</button> : <div className="ai-parse-preview">
            ...
            {plan.mcp_plan?.install ? <div className="ai-parse-evidence"><strong>{t('aiParse.runtimeInstall')}</strong><ul><li><code>{plan.mcp_plan.install.tool}</code> · <code>{plan.mcp_plan.install.packages.join(', ')}</code> · <em>{plan.mcp_plan.install.status}</em>{plan.mcp_plan.runtime ? ` · ${t('aiParse.runtime')}: ${plan.mcp_plan.runtime}` : ''}</li>{plan.mcp_plan.install.detail ? <li className="mono ai-parse-install-detail">{plan.mcp_plan.install.detail}</li> : null}</ul></div> : null}
