      ...
      <div className="modal-body ai-parse-body">
        {!eligible.length ? <div className="ai-parse-notice"><TriangleAlert size={16}/>{t('aiParse.noProvider')}</div> : <>
          ...
          {!plan ? <button type="button" className="btn btn-primary" disabled={busy || !sourceUrl.trim() || !eligible.length} onClick={() => void parse()}><FileSearch size={15}/>{t('aiParse.parse')}</button> : <div className="ai-parse-preview">
            ...
            {plan.mcp_plan?.runtime ? <div className="ai-parse-evidence"><strong>{t('aiParse.runtimeInstall')}</strong><ul><li><code>{plan.mcp_plan.runtime}</code>{plan.mcp_plan.install ? ` · ${plan.mcp_plan.install.tool} · ${plan.mcp_plan.install.packages.join(', ')} · ` : ` · ${t('aiParse.autoFetch')}`}{plan.mcp_plan.install ? <em>{plan.mcp_plan.install.status}</em> : null}{plan.mcp_plan.install?.detail ? <span className="mono ai-parse-install-detail"> · {plan.mcp_plan.install.detail}</span> : null}</li></ul></div> : null}
